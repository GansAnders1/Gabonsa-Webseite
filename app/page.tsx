"use client";

import Link from "next/link";
import {
  useEffect,
  useRef,
  useState,
  type CSSProperties,
  type MouseEvent,
} from "react";

type SectionThreeBackgroundPngSettings = {
  src: string;
  width: string;
  height: string;
  size: string;
  color: string;
  rotation: string;
  opacity: number;
  left?: string;
  right?: string;
  top?: string;
  bottom?: string;
};

type CssVars = CSSProperties & Record<`--${string}`, string>;

type NavLink = {
  href: string;
  label: string;
  external?: boolean;
};

const displayFontFamily =
  'Trattatello, "Marker Felt", "Chalkboard SE", "Comic Sans MS", cursive';

export default function Home() {
  // =========================
  // BEDIENFELD: HEADER-LOGO
  // =========================
  const headerLogoSettings = {
    width: "92px",
    height: "72px",
    color: "#00D9FF",
    moveY: "-8px",
  };

  // =========================
  // BEDIENFELD: MOBILE HEADER
  // =========================
  const mobileHeaderSettings = {
    logoWidth: "58px",
    logoHeight: "47px",
    logoMoveY: "0px",
    textColor: "#f7ffd7",
    textFontSize: "clamp(16px, 4.4vw, 18px)",
  };

  // =========================
  // BEDIENFELD: HEADER-BUTTONS
  // =========================
  const headerButtonSettings = {
    width: "115px",
    height: "36px",
    backgroundColor: "#f7ffd7",
    textColor: "#383F37",
    fontSize: "22px",
  };

  const headerButtonBaseStyle = {
    backgroundColor: headerButtonSettings.backgroundColor,
    color: headerButtonSettings.textColor,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  };

  const headerButtonStyle = {
    ...headerButtonBaseStyle,
    width: headerButtonSettings.width,
    height: headerButtonSettings.height,
    fontSize: headerButtonSettings.fontSize,
  };

  const headerLinks: NavLink[] = [
    { href: "/line-up", label: "Line-Up" },
    { href: "/faq", label: "FAQ" },
    { href: "/ueber-uns", label: "Über uns" },
    { href: "/archive", label: "Archiv" },
    { href: "/partner", label: "Partner" },
    {
      href: "https://forms.gle/iv6jVRm3cU1yua9c8",
      label: "Volunteering",
      external: true,
    },
    { href: "/kontakt", label: "Kontakt" },
  ];

  const mobileBottomRowOne: NavLink[] = [
    { href: "/line-up", label: "Line-Up" },
    { href: "/faq", label: "FAQ" },
    { href: "/ueber-uns", label: "Über uns" },
    { href: "/archive", label: "Archiv" },
  ];

  const mobileBottomRowTwo: NavLink[] = [
    { href: "/partner", label: "Partner" },
    {
      href: "https://forms.gle/iv6jVRm3cU1yua9c8",
      label: "Volunteering",
      external: true,
    },
    { href: "/kontakt", label: "Kontakt" },
  ];

  // =========================
  // BEDIENFELD: MOBILE BUTTONS
  // =========================
  // Die untere mobile Buttonleiste erscheint erst nach dem Scrollen.
  // Die Desktop-Buttons bleiben davon unberührt.
  const mobileButtonSettings = {
    showAfterScroll: true, // true = Leiste erscheint erst nach dem Scrollen
    appearAfterScrollY: "0", // ab wie vielen Scroll-Pixeln die Leiste erscheint
    bottomNavBottom: "52px", // Abstand vom unteren Bildschirmrand
    rowGap: "4px", // Abstand zwischen oberer und unterer Buttonzeile
    columnGap: "4px", // Abstand zwischen Buttons

    buttonMinHeight: "27px", // minimale Höhe der unteren Buttons
    buttonFontSize: "19px", // Schriftgröße der unteren Buttons
    buttonPaddingX: "8px", // Innenabstand links/rechts der unteren Buttons
    buttonPaddingY: "2px", // Innenabstand oben/unten der unteren Buttons
    buttonRadius: "9999px",

    topRowColumns: 4, // erste untere Reihe: 4 Buttons
    bottomRowColumns: 3, // zweite untere Reihe: 3 Buttons
    bottomRowWidth: "82%", // macht die zweite Reihe schmäler
    bottomRowOffset: "9%", // verschiebt die zweite Reihe nach rechts

    mobileNavPaddingX: "14px", // äußerer Abstand links/rechts der unteren Leiste

    topTicketMinHeight: "32px", // minimale Höhe vom Ticketbutton oben rechts
    topTicketFontSize: "22px", // Schriftgröße vom Ticketbutton oben rechts
    topTicketPaddingX: "12px", // Innenabstand links/rechts vom Ticketbutton oben rechts
    topTicketPaddingY: "3px", // Innenabstand oben/unten vom Ticketbutton oben rechts

    backgroundColor: "#f7ffd7",
    textColor: "#383F37",
  };

  // =========================
  // BEDIENFELD: BILD 2 TICKET-SCHRIFTZUG
  // =========================
  const ticketTextSettings = {
    text: "Dein Ticket",
    fontSize: "56px",
    mobileFontSize: "42px",
    color: "#B7F000",
    hoverColor: "#f7ffd7",
    fontFamily: displayFontFamily,
    moveX: "-375px",
  };

  // =========================
  // BEDIENFELD: ABSCHNITT 3
  // =========================
  const sectionThreeSettings = {
    backgroundColor: "#181818",
    textColor: "#EDFCDF",

    fontSize: "38px",
    specialFontSize: "56px",
    mobileFontSize: "24px",
    mobileSpecialFontSize: "36px",

    lineHeight: "1.15",
    padding: "80px",
    mobilePaddingX: "24px",
    mobilePaddingY: "40px",
  };

  // =========================
  // BEDIENFELD: ABSCHNITT 3 BACKGROUND PNG
  // =========================
  const sectionThreeBackgroundPng: SectionThreeBackgroundPngSettings = {
    src: "/startseite-bg-1.png",
    width: "520px",
    height: "520px",
    size: "520px",
    right: "-100px",
    bottom: "-30px",
    color: "#EDFCDF",
    rotation: "-18deg",
    opacity: 0.4,
  };

  // =========================
  // BEDIENFELD: AFTERMOVIE ABSCHNITT
  // =========================
  const aftermovieSettings = {
    videoUrl:
      "https://www.youtube.com/embed/XM5ZW30aWlk?autoplay=1&mute=1&playsinline=1&controls=1&rel=0",

    height: "100vh",
    mobileHeight: "100svh",
    mobileMinHeight: "560px",
    backgroundColor: "#181818",
  };

  // =========================
  // BEDIENFELD: BILD 4 SCHRIFTZUG
  // =========================
  const imageFourTextSettings = {
    text: "Archiv",
    link: "/archive",
    external: false,
    fontSize: "56px",
    mobileFontSize: "42px",
    color: "#EDFCDF",
    fontFamily: displayFontFamily,
    moveX: "375px",
  };

  // =========================
  // BEDIENFELD: ABSCHNITT 5 / LOGO
  // =========================
  const sectionFiveSettings = {
    backgroundColor: "#181818",

    logoSrc: "/Logo-stretched.webp",
    logoColor: "#00D9FF",

    logoWidth: "900px",
    logoHeight: "500px",
    logoMaxWidth: "100svw",
    logoSize: "contain",
    mobileLogoWidth: "96svw",
    mobileLogoHeight: "66svw",
    mobileLogoSize: "contain",

    moveX: "0px",
    moveY: "0px",
  };

  // =========================
  // BEDIENFELD: FOOTER
  // =========================
  const footerSettings = {
    backgroundColor: "#181818",
    textColor: "#EDFCDF",
    borderColor: "#181818",

    fontSize: "14px",
    height: "40px",
    paddingX: "30px",

    fontFamily: "HaasGrotDisp",
    fontWeight: "700",
  };

  const footerNavOne: NavLink[] = [
    {
      href: "https://tickets.nfctron.de/event/gabonsa-kulturverein/gabonsa-festival-2026",
      label: "Tickets",
      external: true,
    },
    { href: "/line-up", label: "Musik" },
    { href: "/faq", label: "FAQ" },
    {
      href: "https://www.youtube.com/watch?v=XM5ZW30aWlk",
      label: "Aftermovie",
      external: true,
    },
    { href: "/partner", label: "Partner" },
    {
      href: "https://forms.gle/iv6jVRm3cU1yua9c8",
      label: "Volunteering",
      external: true,
    },
  ];

  const footerNavTwo: NavLink[] = [
    { href: "/ueber-uns", label: "Werte" },
    { href: "/ueber-uns", label: "Über uns" },
    { href: "/archive", label: "Archiv" },
    { href: "/kontakt", label: "Kontakt" },
    { href: "/impressum", label: "Impressum" },
  ];

  const footerNavThree: NavLink[] = [
    {
      href: "https://www.instagram.com/gabonsa.festival",
      label: "Instagram",
      external: true,
    },
    {
      href: "https://de.ra.co/events/2417850",
      label: "Resident Advisor",
      external: true,
    },
    { href: "https://linktr.ee/gabonsa", label: "Linktree", external: true },
  ];

  const mobileFooterRowOne: NavLink[] = [
    { href: "/archive", label: "Archiv" },
    { href: "/partner", label: "Partner" },
    { href: "/kontakt", label: "Kontakt" },
    {
      href: "https://tickets.nfctron.de/event/gabonsa-kulturverein/gabonsa-festival-2026",
      label: "Tickets",
      external: true,
    },
    { href: "/impressum", label: "Impressum" },
  ];

  const mobileFooterRowTwo: NavLink[] = [
    { href: "/ueber-uns", label: "Werte" },
    {
      href: "https://forms.gle/iv6jVRm3cU1yua9c8",
      label: "Volunteer",
      external: true,
    },
    {
      href: "https://www.instagram.com/gabonsa.festival",
      label: "Instagram",
      external: true,
    },
    {
      href: "https://www.youtube.com/watch?v=XM5ZW30aWlk",
      label: "YouTube",
      external: true,
    },
    { href: "https://linktr.ee/gabonsa", label: "Linktree", external: true },
  ];

  const [hasStartedScrolling, setHasStartedScrolling] = useState(false);
  const [isMobileBottomNavOverFooter, setIsMobileBottomNavOverFooter] =
    useState(false);
  const footerRef = useRef<HTMLElement | null>(null);
  const mobileBottomNavRef = useRef<HTMLElement | null>(null);

  useEffect(() => {
    const scrollThreshold = Number(mobileButtonSettings.appearAfterScrollY);
    const getScrollY = () =>
      window.scrollY ||
      document.documentElement.scrollTop ||
      document.body.scrollTop ||
      0;
    const updateScrollState = () => {
      setHasStartedScrolling(
        (hasStarted) => hasStarted || getScrollY() > scrollThreshold
      );
    };
    const markScrollStarted = () => setHasStartedScrolling(true);
    const visualViewport = window.visualViewport;

    updateScrollState();
    window.addEventListener("scroll", updateScrollState, { passive: true });
    window.addEventListener("wheel", markScrollStarted, { passive: true });
    window.addEventListener("touchstart", markScrollStarted, {
      passive: true,
    });
    window.addEventListener("touchmove", markScrollStarted, { passive: true });
    window.addEventListener("pointermove", updateScrollState, {
      passive: true,
    });
    visualViewport?.addEventListener("scroll", updateScrollState, {
      passive: true,
    });
    visualViewport?.addEventListener("resize", updateScrollState);

    return () => {
      window.removeEventListener("scroll", updateScrollState);
      window.removeEventListener("wheel", markScrollStarted);
      window.removeEventListener("touchstart", markScrollStarted);
      window.removeEventListener("touchmove", markScrollStarted);
      window.removeEventListener("pointermove", updateScrollState);
      visualViewport?.removeEventListener("scroll", updateScrollState);
      visualViewport?.removeEventListener("resize", updateScrollState);
    };
  }, [mobileButtonSettings.appearAfterScrollY]);

  useEffect(() => {
    const updateFooterOverlap = () => {
      const footerElement = footerRef.current;
      const navElement = mobileBottomNavRef.current;

      if (!footerElement || !navElement) {
        setIsMobileBottomNavOverFooter(false);
        return;
      }

      const footerRect = footerElement.getBoundingClientRect();
      const navRect = navElement.getBoundingClientRect();
      const viewportHeight = window.visualViewport?.height ?? window.innerHeight;
      const footerVisible = footerRect.top < viewportHeight && footerRect.bottom > 0;
      const navOverFooter =
        footerRect.top < navRect.bottom && footerRect.bottom > navRect.top;

      setIsMobileBottomNavOverFooter(footerVisible || navOverFooter);
    };

    updateFooterOverlap();
    window.addEventListener("scroll", updateFooterOverlap, { passive: true });
    window.addEventListener("resize", updateFooterOverlap);

    return () => {
      window.removeEventListener("scroll", updateFooterOverlap);
      window.removeEventListener("resize", updateFooterOverlap);
    };
  }, []);

  const shouldShowMobileBottomNav =
    !isMobileBottomNavOverFooter &&
    (!mobileButtonSettings.showAfterScroll || hasStartedScrolling);

  const handleHomeLogoClick = (event: MouseEvent<HTMLAnchorElement>) => {
    if (window.location.pathname !== "/") return;

    event.preventDefault();
    window.scrollTo({ top: 0, behavior: "smooth" });
  };

  const renderLink = ({
    href,
    label,
    external,
  }: {
    href: string;
    label: string;
    external?: boolean;
  }) => (
    <a
      key={`${href}-${label}`}
      href={href}
      target={external ? "_blank" : undefined}
      rel={external ? "noopener noreferrer" : undefined}
    >
      {label}
    </a>
  );

  return (
    <main className="min-h-screen overflow-x-hidden bg-black text-white">
      <style>
        {`
          @font-face {
            font-family: "HaasGrotDisp";
            src: url("/HaasGrotDisp-65Medium.otf") format("opentype");
            font-weight: 500;
            font-style: normal;
            font-display: swap;
          }

          .home-header-logo {
            width: ${mobileHeaderSettings.logoWidth};
            height: ${mobileHeaderSettings.logoHeight};
            transform: translateY(${mobileHeaderSettings.logoMoveY});
          }

          .desktop-offset-link {
            transform: translate(-50%, -50%);
          }

          .section-three-bg-png {
            width: min(68vw, 300px);
            height: min(68vw, 300px);
            right: -70px;
            bottom: 12px;
            -webkit-mask-size: contain;
            mask-size: contain;
          }

          .mobile-bottom-nav {
            bottom: max(${mobileButtonSettings.bottomNavBottom}, calc(env(safe-area-inset-bottom) + ${mobileButtonSettings.bottomNavBottom}));
          }

          .section-five-logo {
            -webkit-mask-size: ${sectionFiveSettings.mobileLogoSize};
            mask-size: ${sectionFiveSettings.mobileLogoSize};
          }

          @media (min-width: 768px) {
            .home-header-logo {
              width: ${headerLogoSettings.width};
              height: ${headerLogoSettings.height};
              transform: translateY(${headerLogoSettings.moveY});
            }

            .section-five-logo {
              -webkit-mask-size: ${sectionFiveSettings.logoSize};
              mask-size: ${sectionFiveSettings.logoSize};
            }
          }

          @media (min-width: 1024px) {
            .desktop-offset-link {
              transform: translate(calc(-50% + var(--desktop-move-x)), -50%);
            }

            .section-three-bg-png {
              width: ${sectionThreeBackgroundPng.width};
              height: ${sectionThreeBackgroundPng.height};
              right: ${sectionThreeBackgroundPng.right};
              bottom: ${sectionThreeBackgroundPng.bottom};
              -webkit-mask-size: ${sectionThreeBackgroundPng.size};
              mask-size: ${sectionThreeBackgroundPng.size};
            }
          }
        `}
      </style>

      {/* ========================= */}
      {/* ERSTER BEREICH / LANDING */}
      {/* ========================= */}
      <section className="relative h-[100svh] min-h-[620px] overflow-hidden bg-black text-white md:h-screen md:min-h-0">
        <div
          className="absolute inset-0 bg-cover bg-no-repeat"
          style={{
            backgroundImage: "url('/festival-bg.jpg')",
            backgroundPosition: "center 82%",
          }}
        />

        <div className="absolute inset-0 bg-black/25" />
        <div className="absolute inset-0 bg-pink-500/20 mix-blend-screen" />

        <header
          className="fixed left-0 top-0 z-50 grid w-full grid-cols-[58px_minmax(0,1fr)_96px] items-start gap-x-2 px-2.5 py-5 min-[390px]:grid-cols-[58px_minmax(0,1fr)_102px] md:flex md:items-start md:justify-between md:px-6 md:py-5"
          style={{
            fontFamily: displayFontFamily,
          }}
        >
          <div className="contents md:flex md:min-w-0 md:items-start md:gap-4">
            <Link
              href="/"
              aria-label="Zur Startseite"
              onClick={handleHomeLogoClick}
              className="relative z-10 block shrink-0 cursor-pointer"
            >
              <div
                className="home-header-logo shrink-0"
                style={{
                  backgroundColor: headerLogoSettings.color,

                  WebkitMaskImage: "url('/Gabonsa_logo-02.png')",
                  maskImage: "url('/Gabonsa_logo-02.png')",
                  WebkitMaskRepeat: "no-repeat",
                  maskRepeat: "no-repeat",
                  WebkitMaskPosition: "center",
                  maskPosition: "center",
                  WebkitMaskSize: "contain",
                  maskSize: "contain",
                }}
              />
            </Link>

            <div
              className="block min-w-0 overflow-visible pt-0 text-left leading-[1.04] drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] md:hidden"
              style={{
                color: mobileHeaderSettings.textColor,
                fontSize: mobileHeaderSettings.textFontSize,
              }}
            >
              <p>Gabonsa Festival</p>
              <p className="whitespace-nowrap tracking-[-0.03em]">
                03. &amp; 04.07.2026, Innsbruck
              </p>
            </div>

            <nav className="hidden items-center gap-3 md:flex">
              {headerLinks.map((link) => (
                <a
                  key={`${link.href}-${link.label}`}
                  href={link.href}
                  target={link.external ? "_blank" : undefined}
                  rel={link.external ? "noopener noreferrer" : undefined}
                  className="rounded-full shadow-md transition hover:scale-105"
                  style={headerButtonStyle}
                >
                  {link.label}
                </a>
              ))}
            </nav>
          </div>

          <a
            href="https://tickets.nfctron.de/event/gabonsa-kulturverein/gabonsa-festival-2026"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex min-h-[var(--mobile-top-ticket-min-height)] w-auto shrink-0 items-center justify-center rounded-full px-[var(--mobile-top-ticket-padding-x)] py-[var(--mobile-top-ticket-padding-y)] text-[length:var(--mobile-top-ticket-font-size)] leading-none shadow-md transition hover:scale-105 md:h-[var(--desktop-header-button-height)] md:w-[var(--desktop-header-button-width)] md:px-0 md:py-0 md:text-[length:var(--desktop-header-button-font-size)]"
            style={
              {
                ...headerButtonBaseStyle,
                "--mobile-top-ticket-min-height":
                  mobileButtonSettings.topTicketMinHeight,
                "--mobile-top-ticket-padding-x":
                  mobileButtonSettings.topTicketPaddingX,
                "--mobile-top-ticket-padding-y":
                  mobileButtonSettings.topTicketPaddingY,
                "--mobile-top-ticket-font-size":
                  mobileButtonSettings.topTicketFontSize,
                "--desktop-header-button-height": headerButtonSettings.height,
                "--desktop-header-button-width": headerButtonSettings.width,
                "--desktop-header-button-font-size":
                  headerButtonSettings.fontSize,
              } as CssVars
            }
          >
            Tickets
          </a>
        </header>

        <nav
          ref={mobileBottomNavRef}
          className={`mobile-bottom-nav fixed inset-x-0 z-50 flex flex-col items-stretch transition duration-300 md:hidden ${
            shouldShowMobileBottomNav
              ? "translate-y-0 opacity-100"
              : "pointer-events-none translate-y-8 opacity-0"
          }`}
          style={{
            paddingLeft: mobileButtonSettings.mobileNavPaddingX,
            paddingRight: mobileButtonSettings.mobileNavPaddingX,
            gap: mobileButtonSettings.rowGap,
            fontFamily: displayFontFamily,
          }}
          aria-label="Mobile Hauptnavigation"
        >
          <div
            className="grid w-full items-center"
            style={{
              gridTemplateColumns: `repeat(${mobileButtonSettings.topRowColumns}, minmax(0, 1fr))`,
              gap: mobileButtonSettings.columnGap,
            }}
          >
            {mobileBottomRowOne.map((link) => (
              <a
                key={`${link.href}-${link.label}-mobile-bottom-row-one`}
                href={link.href}
                target={link.external ? "_blank" : undefined}
                rel={link.external ? "noopener noreferrer" : undefined}
                className="inline-flex w-full min-w-0 items-center justify-center whitespace-nowrap shadow-md transition hover:scale-105"
                style={{
                  minHeight: mobileButtonSettings.buttonMinHeight,
                  borderRadius: mobileButtonSettings.buttonRadius,
                  paddingLeft: mobileButtonSettings.buttonPaddingX,
                  paddingRight: mobileButtonSettings.buttonPaddingX,
                  paddingTop: mobileButtonSettings.buttonPaddingY,
                  paddingBottom: mobileButtonSettings.buttonPaddingY,
                  fontSize: mobileButtonSettings.buttonFontSize,
                  lineHeight: 1,
                  backgroundColor: mobileButtonSettings.backgroundColor,
                  color: mobileButtonSettings.textColor,
                }}
              >
                {link.label}
              </a>
            ))}
          </div>

          <div
            className="grid items-center"
            style={{
              width: mobileButtonSettings.bottomRowWidth,
              marginLeft: mobileButtonSettings.bottomRowOffset,
              gridTemplateColumns: `repeat(${mobileButtonSettings.bottomRowColumns}, minmax(0, 1fr))`,
              gap: mobileButtonSettings.columnGap,
            }}
          >
            {mobileBottomRowTwo.map((link) => (
              <a
                key={`${link.href}-${link.label}-mobile-bottom-row-two`}
                href={link.href}
                target={link.external ? "_blank" : undefined}
                rel={link.external ? "noopener noreferrer" : undefined}
                className="inline-flex w-full min-w-0 items-center justify-center whitespace-nowrap shadow-md transition hover:scale-105"
                style={{
                  minHeight: mobileButtonSettings.buttonMinHeight,
                  borderRadius: mobileButtonSettings.buttonRadius,
                  paddingLeft: mobileButtonSettings.buttonPaddingX,
                  paddingRight: mobileButtonSettings.buttonPaddingX,
                  paddingTop: mobileButtonSettings.buttonPaddingY,
                  paddingBottom: mobileButtonSettings.buttonPaddingY,
                  fontSize: mobileButtonSettings.buttonFontSize,
                  lineHeight: 1,
                  backgroundColor: mobileButtonSettings.backgroundColor,
                  color: mobileButtonSettings.textColor,
                }}
              >
                {link.label}
              </a>
            ))}
          </div>
        </nav>

        <div className="absolute inset-0 z-10 flex items-center justify-center px-5">
          <Link
            href="/"
            aria-label="Gabonsa Startseite"
            onClick={handleHomeLogoClick}
            className="block cursor-pointer"
          >
            <img
              src="/gabonsa-logo.webp"
              alt="Gabonsa Logo"
              className="w-[820px] max-w-[96vw] translate-y-[-28px] drop-shadow-[0_10px_35px_rgba(0,0,0,0.75)] md:w-[1700px] md:max-w-[95vw] md:translate-y-[-20px]"
            />
          </Link>
        </div>

        <a
          href="https://www.youtube.com/watch?v=XM5ZW30aWlk"
          target="_blank"
          rel="noopener noreferrer"
          className="absolute left-1/2 top-[57%] z-20 -translate-x-1/2 rounded-full bg-cyan-400 px-5 py-1.5 text-[18px] text-black shadow-lg transition hover:scale-105 hover:bg-cyan-300 md:top-[70%] md:px-8 md:py-2 md:text-[24px]"
          style={{
            fontFamily: displayFontFamily,
          }}
        >
          Video
        </a>

        <div
          className="absolute bottom-8 left-8 z-20 hidden text-left text-[35px] leading-tight text-[#f7ffd7] drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] md:block"
          style={{
            fontFamily: displayFontFamily,
          }}
        >
          <p>Gabonsa Festival</p>
          <p>03. &amp; 04.07.2026, Innsbruck</p>
        </div>

        <div
          className="absolute bottom-8 right-8 z-20 hidden text-right text-[35px] leading-tight text-[#f7ffd7] drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)] md:block"
          style={{
            fontFamily: displayFontFamily,
          }}
        >
          <p>Stell dir vor es ist Zukunft</p>
          <p>und alles ist gut.</p>
        </div>
      </section>

      {/* ========================= */}
      {/* ZWEITER BEREICH / BILD 2 */}
      {/* ========================= */}
      <section className="relative h-[100svh] min-h-[560px] overflow-hidden bg-black md:h-screen md:min-h-0">
        <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{
            backgroundImage: "url('/Bild2.jpg')",
          }}
        />

        <div className="absolute inset-0 bg-black/15" />

        <a
          href="https://tickets.nfctron.de/event/gabonsa-kulturverein/gabonsa-festival-2026"
          target="_blank"
          rel="noopener noreferrer"
          className="desktop-offset-link absolute left-1/2 top-1/2 z-30 whitespace-nowrap text-[length:var(--mobile-ticket-font-size)] transition hover:scale-105 md:text-[length:var(--desktop-ticket-font-size)]"
          style={
            {
              "--desktop-move-x": ticketTextSettings.moveX,
              "--mobile-ticket-font-size": ticketTextSettings.mobileFontSize,
              "--desktop-ticket-font-size": ticketTextSettings.fontSize,
              color: ticketTextSettings.color,
              fontFamily: ticketTextSettings.fontFamily,
            } as CssVars
          }
        >
          {ticketTextSettings.text}
        </a>
      </section>

      {/* ========================= */}
      {/* DRITTER BEREICH / BILD 3 */}
      {/* ========================= */}
      <section className="relative grid min-h-[100svh] grid-cols-1 overflow-hidden bg-black md:h-screen md:grid-cols-2">
        <div
          className="h-[42svh] min-h-[280px] w-full bg-cover bg-center bg-no-repeat md:h-full md:min-h-0"
          style={{
            backgroundImage: "url('/Bild3.jpg')",
          }}
        />

        <div
          className="relative flex min-h-[58svh] items-center overflow-hidden px-[var(--section-three-mobile-padding-x)] py-[var(--section-three-mobile-padding-y)] md:h-full md:min-h-0 md:p-[var(--section-three-desktop-padding)]"
          style={
            {
              backgroundColor: sectionThreeSettings.backgroundColor,
              "--section-three-mobile-padding-x":
                sectionThreeSettings.mobilePaddingX,
              "--section-three-mobile-padding-y":
                sectionThreeSettings.mobilePaddingY,
              "--section-three-desktop-padding": sectionThreeSettings.padding,
            } as CssVars
          }
        >
          <div
            className="section-three-bg-png pointer-events-none absolute z-0 select-none"
            style={{
              opacity: sectionThreeBackgroundPng.opacity,
              backgroundColor: sectionThreeBackgroundPng.color,
              transform: `rotate(${sectionThreeBackgroundPng.rotation})`,

              WebkitMaskImage: `url('${sectionThreeBackgroundPng.src}')`,
              maskImage: `url('${sectionThreeBackgroundPng.src}')`,
              WebkitMaskRepeat: "no-repeat",
              maskRepeat: "no-repeat",
              WebkitMaskPosition: "center",
              maskPosition: "center",
            }}
          />

          <p
            className="relative z-10 text-[length:var(--section-three-mobile-font-size)] leading-[1.15] md:text-[length:var(--section-three-desktop-font-size)]"
            style={
              {
                "--section-three-mobile-font-size":
                  sectionThreeSettings.mobileFontSize,
                "--section-three-desktop-font-size":
                  sectionThreeSettings.fontSize,
                color: sectionThreeSettings.textColor,
                lineHeight: sectionThreeSettings.lineHeight,
                fontFamily: "HaasGrotDisp",
              } as CssVars
            }
          >
            <span
              className="text-[length:var(--section-three-mobile-special-font-size)] md:text-[length:var(--section-three-desktop-special-font-size)]"
              style={
                {
                  "--section-three-mobile-special-font-size":
                    sectionThreeSettings.mobileSpecialFontSize,
                  "--section-three-desktop-special-font-size":
                    sectionThreeSettings.specialFontSize,
                  fontFamily: displayFontFamily,
                } as CssVars
              }
            >
              Gabonsa 2026
            </span>{" "}
            - eine{" "}
            <span
              className="text-[length:var(--section-three-mobile-special-font-size)] md:text-[length:var(--section-three-desktop-special-font-size)]"
              style={
                {
                  "--section-three-mobile-special-font-size":
                    sectionThreeSettings.mobileSpecialFontSize,
                  "--section-three-desktop-special-font-size":
                    sectionThreeSettings.specialFontSize,
                  fontFamily: displayFontFamily,
                } as CssVars
              }
            >
              Symbiose
            </span>
            . Musik, Installation und Performance. Umgeben von Bergen entsteht
            ein Raum zwischen{" "}
            <span
              className="text-[length:var(--section-three-mobile-special-font-size)] md:text-[length:var(--section-three-desktop-special-font-size)]"
              style={
                {
                  "--section-three-mobile-special-font-size":
                    sectionThreeSettings.mobileSpecialFontSize,
                  "--section-three-desktop-special-font-size":
                    sectionThreeSettings.specialFontSize,
                  fontFamily: displayFontFamily,
                } as CssVars
              }
            >
              Gegenwart
            </span>{" "}
            und{" "}
            <span
              className="text-[length:var(--section-three-mobile-special-font-size)] md:text-[length:var(--section-three-desktop-special-font-size)]"
              style={
                {
                  "--section-three-mobile-special-font-size":
                    sectionThreeSettings.mobileSpecialFontSize,
                  "--section-three-desktop-special-font-size":
                    sectionThreeSettings.specialFontSize,
                  fontFamily: displayFontFamily,
                } as CssVars
              }
            >
              Zukunft
            </span>
            . Ein{" "}
            <span
              className="text-[length:var(--section-three-mobile-special-font-size)] md:text-[length:var(--section-three-desktop-special-font-size)]"
              style={
                {
                  "--section-three-mobile-special-font-size":
                    sectionThreeSettings.mobileSpecialFontSize,
                  "--section-three-desktop-special-font-size":
                    sectionThreeSettings.specialFontSize,
                  fontFamily: displayFontFamily,
                } as CssVars
              }
            >
              Möglichkeitsraum
            </span>
            .
          </p>
        </div>
      </section>

      {/* ========================= */}
      {/* NEUER BEREICH / AFTERMOVIE */}
      {/* ========================= */}
      <section
        className="relative h-[var(--aftermovie-mobile-height)] min-h-[var(--aftermovie-mobile-min-height)] overflow-hidden md:h-[var(--aftermovie-desktop-height)]"
        style={
          {
            "--aftermovie-mobile-height": aftermovieSettings.mobileHeight,
            "--aftermovie-mobile-min-height": aftermovieSettings.mobileMinHeight,
            "--aftermovie-desktop-height": aftermovieSettings.height,
            backgroundColor: aftermovieSettings.backgroundColor,
          } as CssVars
        }
      >
        <iframe
          className="absolute left-1/2 top-1/2 h-[100vh] w-[177.78vh] min-h-[56.25vw] min-w-[100vw] -translate-x-1/2 -translate-y-1/2"
          src={aftermovieSettings.videoUrl}
          title="Gabonsa Aftermovie"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          allowFullScreen
        />
      </section>

      {/* ========================= */}
      {/* VIERTER BEREICH / BILD 4 */}
      {/* ========================= */}
      <section className="relative h-[100svh] min-h-[560px] overflow-hidden bg-black md:h-screen md:min-h-0">
        <div
          className="absolute inset-0 bg-cover bg-center bg-no-repeat"
          style={{
            backgroundImage: "url('/Bild4.jpg')",
          }}
        />

        <div className="absolute inset-0 bg-black/15" />

        <a
          href={imageFourTextSettings.link}
          target={imageFourTextSettings.external ? "_blank" : undefined}
          rel={imageFourTextSettings.external ? "noopener noreferrer" : undefined}
          className="desktop-offset-link absolute left-1/2 top-1/2 z-30 whitespace-nowrap text-[length:var(--mobile-image-four-font-size)] transition hover:scale-105 md:text-[length:var(--desktop-image-four-font-size)]"
          style={
            {
              "--desktop-move-x": imageFourTextSettings.moveX,
              "--mobile-image-four-font-size":
                imageFourTextSettings.mobileFontSize,
              "--desktop-image-four-font-size": imageFourTextSettings.fontSize,
              color: imageFourTextSettings.color,
              fontFamily: imageFourTextSettings.fontFamily,
            } as CssVars
          }
        >
          {imageFourTextSettings.text}
        </a>
      </section>

      {/* ========================= */}
      {/* FÜNFTER BEREICH / LOGO */}
      {/* ========================= */}
      <section
        className="relative flex h-[100svh] min-h-[560px] items-center justify-center overflow-hidden md:h-screen"
        style={{
          backgroundColor: sectionFiveSettings.backgroundColor,
        }}
      >
        <div
          aria-label="Gabonsa Logo"
          className="section-five-logo h-[var(--section-five-mobile-logo-height)] w-[var(--section-five-mobile-logo-width)] md:h-[var(--section-five-desktop-logo-height)] md:w-[var(--section-five-desktop-logo-width)] md:max-w-[var(--section-five-desktop-logo-max-width)]"
          style={
            {
              "--section-five-mobile-logo-height":
                sectionFiveSettings.mobileLogoHeight,
              "--section-five-mobile-logo-width":
                sectionFiveSettings.mobileLogoWidth,
              "--section-five-desktop-logo-height":
                sectionFiveSettings.logoHeight,
              "--section-five-desktop-logo-width":
                sectionFiveSettings.logoWidth,
              "--section-five-desktop-logo-max-width":
                sectionFiveSettings.logoMaxWidth,
              backgroundColor: sectionFiveSettings.logoColor,
              transform: `translate(${sectionFiveSettings.moveX}, ${sectionFiveSettings.moveY})`,

              WebkitMaskImage: `url('${sectionFiveSettings.logoSrc}')`,
              maskImage: `url('${sectionFiveSettings.logoSrc}')`,
              WebkitMaskRepeat: "no-repeat",
              maskRepeat: "no-repeat",
              WebkitMaskPosition: "center",
              maskPosition: "center",
            } as CssVars
          }
        />
      </section>

      {/* ========================= */}
      {/* FOOTER */}
      {/* ========================= */}
      <footer
        ref={footerRef}
        data-site-footer
        className="w-full border-t px-3 py-5 md:flex md:h-[var(--footer-height)] md:items-center md:justify-between md:gap-6 md:px-[var(--footer-padding-x)] md:py-0"
        style={
          {
            "--footer-height": footerSettings.height,
            "--footer-padding-x": footerSettings.paddingX,
            backgroundColor: footerSettings.backgroundColor,
            color: footerSettings.textColor,
            borderColor: footerSettings.borderColor,
            fontSize: footerSettings.fontSize,
            fontFamily: footerSettings.fontFamily,
            fontWeight: footerSettings.fontWeight,
          } as CssVars
        }
      >
        <div className="grid gap-2 text-[12px] leading-tight md:hidden">
          <nav className="grid grid-cols-5 items-start gap-x-2 gap-y-1 text-center">
            {mobileFooterRowOne.map(renderLink)}
          </nav>
          <nav className="grid grid-cols-5 items-start gap-x-2 gap-y-1 text-center">
            {mobileFooterRowTwo.map(renderLink)}
          </nav>
        </div>

        <div className="hidden whitespace-nowrap md:block">
          © Gabonsa Festival
        </div>

        <nav className="hidden items-center gap-5 whitespace-nowrap md:flex">
          {footerNavOne.map(renderLink)}
        </nav>

        <nav className="hidden items-center gap-5 whitespace-nowrap md:flex">
          {footerNavTwo.map(renderLink)}
        </nav>

        <nav className="hidden items-center gap-5 whitespace-nowrap md:flex">
          {footerNavThree.map(renderLink)}
        </nav>
      </footer>
    </main>
  );
}
